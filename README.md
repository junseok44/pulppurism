# 두런두런 옥천마루

옥천마루는 주민의 일상적인 의견을 모아 정책 안건으로 정리하고, 실제 정책 제안과 실행 과정까지 연결하는 주민 참여 기반 정책 제안 플랫폼입니다. 

## 프로젝트 개요

### 문제 상황

- 지방이 거주하기 좋은 공간이 되기 위해서는 주민의 삶에 밀착된 정책이 필요하지만, 이를 지속적으로 제안하고 논의할 수 있는 지역 차원의 공론장이 부족하다.
- 주민들은 정책에 대한 구체적인 니즈를 가지고 있으나, 개인이 이를 공식적인 건의로 연결하기에는 심리적·행정적 부담이 크다.
- 마을 내 소통은 비교적 활발하지만, 주민 의견이 객관적인 데이터로 축적되지 않아 행정을 설득할 근거가 부족하며, 개개인이 유사한 정책 사례나 선례를 찾기도 어렵다.
- 그 결과, 반복적으로 제기되는 **주민 수요가 정책으로 연결되지 못하고 단발적인 의견에 그치는 구조**가 형성되어 있다.

### 해결 아이디어

- 주민 의견을 음성·텍스트로 쉽게 입력받고, 비슷한 의견들을 AI로 분류·클러스터링하여 하나의 ‘안건’으로 정제한다.
- 안건 단위로 주민 공감도(좋아요, 댓글, 투표), 참고 자료, 타 지역 사례를 함께 축적해 정책 제안에 필요한 근거를 마련한다.
- 의견 → 안건 → 정책 제안 → 실행까지의 전 과정을 타임라인으로 공개해 정책 흐름을 투명하게 보여주고, 정책 실행 현황에 대한 꾸준한 피드백을 제공한다.

### 기대 효과

- 지역 문제를 효율적으로 해결한다.
- 지역 공동체 내 신뢰와 협력이 강화된다.
- 보도 및 행정 전반에서 부담이 경감된다.
- 정책 수립과 보도의 효율성과 정확성이 강화된다.

### 핵심 가치

- 접근성 : 누구나 가볍게 의견을 남길 수 있다.
- 축적성 : 주민 의견이 휘발되지 않고 기록되어 남는다.
- 연결성 : 개인의 의견을 공동의 안건과 정책으로 연결된다.
- 투명성 : 정책 제안과 실행 과정을 주민에게 지속적으로 공유한다.

## 주요 기능 및 사용 플로우

### 주민(일반 유저)

1. 의견 제안
    - ‘제안하기’ 버튼을 눌러 채팅창을 활성화환다.
    - 텍스트 또는 음성 입력을 통해 일상적인 불편 또는 필요를 자유롭게 제안한다.

2. 공감 및 상호작용
    - 다른 주민의 의견에 좋아요, 댓글을 표시한다.

3. 안건 확인 및 참여
    - 비슷한 의견이 모여 생성된 안건을 확인한다.
    - 참고 자료, 주민 의견, 타임라인 등을 확인한다.
    - 투표로 자신의 의사를 표현한다.

4. 정책 진행 상황 및 결과 확인
    - 안건이 정책으로 통과되어 실행되는 과정 또는 완료된 결과를 확인한다.

### 관리자

1. 주민 의견 관리
    - AI 자동 클러스터링(수동 가능)
2. 안건 관리
    - 안건 생성 · 참고자료 업로드 · 타임라인 업데이트
3. 사용자 관리
    - 사용자 조회 · 제재 · 관리자 권한 관리

## 데모 및 자료 링크

- 배포 링크: https://ocmaru.com
- 시연 영상: https://www.youtube.com/watch?v=I_FmlQDKaLA&feature=youtu.be
- 발표 자료: 

## 기술 스택

### Frontend
- **React 18** 
- **TypeScript** 
- **Google Analytics 4 (GA4)**


### Backend
- **Node.js**
- **Express** 
- **TypeScript**

### Database
- **PostgreSQL** 
- **Drizzle ORM** 

### AI
- **OpenAI API** - 임베딩 및 텍스트 생성 (의견 클러스터링, 제목/요약 생성)

### Infrastructure
- **Docker** 
- **Neon** - database hosting
- **Google Cloud Run** - 서버리스 컨테이너 배포
- **Google Cloud Scheduler** - 스케줄링 (클러스터링 작업)
- **Google Cloud Storage** - 파일 저장소
- **Google Cloud Build** - CI/CD

## 서비스 아키텍처



## 코드 설치 및 실행 방법

### 필수 요구사항
- Node.js 20 이상
- PostgreSQL 데이터베이스 (또는 Neon 계정)
- npm 또는 yarn

### 설치

```bash
# 의존성 설치
npm install
```

### 환경 변수 설정

프로젝트 루트에 `.env` 파일을 생성하고 다음 변수들을 설정하세요:

```env
# 데이터베이스
DATABASE_URL=your_postgresql_connection_string

# 세션
SESSION_SECRET=your_session_secret

# 포트
PORT=5000

# OpenAI
OPENAI_API_KEY=your_openai_api_key

# OAuth (Google, Kakao)
GOOGLE_CLIENT_ID=your_google_client_id
GOOGLE_CLIENT_SECRET=your_google_client_secret

KAKAO_CLIENT_ID=your_kakao_client_id
KAKAO_CLIENT_SECRET=your_kakao_client_secret

BASE_URL=http://localhost:5000
```

### 실행

```bash
# 개발 모드 실행
npm run dev

# 프로덕션 빌드
npm run build

# 프로덕션 실행
npm start
```

### Docker를 통한 실행

#### Docker 이미지 빌드

```bash
# Docker 이미지 빌드
docker build -t pulppurism .
```

#### Docker 컨테이너 실행

```bash
# 환경 변수 파일(.env)을 사용하여 컨테이너 실행
docker run -d \
  --name pulppurism \
  -p 5000:5000 \
  --env-file .env \
  pulppurism
```




## 팀 소개

| 이름 | 역할 | 담당 업무 |
|------|------|----------|
| 박지은 | 팀장 | 일정 관리 및 외부 협력 |
| 한승현 | 기획 | 요구사항 분석 및 기능 기획 |
| 김예랑 | UI/UX | 사용자별 유저 플로우 설계 |
| 장준석 | 백엔드 | 백엔드 개발 및 인프라 구축 |
| 윤해리 | 프런트엔드 | 클라이언트 및 디자인 구현 |


## 협력기관, 참여 펠로우 멘토

### 4-1. 협력 기관

- 옥천신문
- 카카오 임팩트

### 4-2. 펠로우 & 멘토

- **Fellow**: 황민호 님 (옥천신문 Okinews)
- **Mentor**: 김평안 님 (카카오 Kakao)

## 확장 방향

- 주민 의견 태그(Tag) 도입
- 디지털 소외 계층을 위한 레거시 채널(전화·문자) 연동 옴니채널 구축
- 위치 기반(LBS) 지역 세분화 및 하이퍼로컬(Hyper-local) 서비스 적용
- 생성형 AI(LLM) 기반 안건 작성 어시스턴트(Writing Assistant) 도입

## 저작권 및 라이선스

본 프로젝트는 MIT 라이선스 하에 배포됩니다.

Copyright (c) 2025 풀뿌리즘
Developed with support from Kakao Impact Tech for Impact Program.

### Acknowledgement

본 프로젝트는 카카오임팩트 재단의 지원과 테크포임팩트 커뮤니티의 기여로 개발되었습니다.

This Service was developed with support from kakaoimpact Foundation and contributions from the Tech for Impact community.

- [테크포임팩트 홈페이지](https://techforimpact.io/)

